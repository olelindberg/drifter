...
| **A8** pre-computed sparsity | Triplets + `setFromTriplets` on every assembly — [cg_smoother_base.cpp:301](../../src/bathymetry/cg_smoother_base.cpp#L301), [:381](../../src/bathymetry/cg_smoother_base.cpp#L381) | **Gap (minor).** No reuse of a precomputed pattern, but each AMR iteration rebuilds the mesh and DOF numbering, so the pattern changes anyway; within a single solve `setFromTriplets` is standard. |
...
| Finding | Code path | Verdict |
|---|---|---|
| **B1** matrix-free ≫ matrix-based | Everything assembled (`H_global_`, `BtWB_global_`, `Q`) + direct **SparseLU** by default — [cg_smoother_base.cpp:396](../../src/bathymetry/cg_smoother_base.cpp#L396), [cg_cubic_bezier_bathymetry_smoother.cpp:261](../../src/bathymetry/cg_cubic_bezier_bathymetry_smoother.cpp#L261) | **Deviation, justified at scale.** This is the matrix-based path the paper argues against — but the paper's argument is about *large iterative* solves. At ≤~10⁵ DOF a direct sparse factorization is typically optimal, so the **default is well-chosen**. B1 becomes relevant only if the problem is scaled up or the optional iterative path is used at scale. |
| **B2** sum factorization | Data fitting forms dense 16×16 BᵀWB per Gauss point; `basis().evaluate(u,v)` recomputed at fixed reference points inside the per-element loop — [cg_smoother_base.cpp:336-364](../../src/bathymetry/cg_smoother_base.cpp#L336) | **Gap (minor here).** The reference basis tables are element-independent and could be precomputed once; sum factorization itself only matters if the operator goes matrix-free. |
| **B3** even-odd decomposition | not used | **N-A** (only relevant with sum-factorized matrix-free kernels). |
| **B4** affine Jacobian reuse | Thin-plate Hessian computed once, reused via `scaled_hessian(dx,dy)` — [cg_smoother_base.cpp:281](../../src/bathymetry/cg_smoother_base.cpp#L281); element-matrix cache keyed on (morton, level) — [:431](../../src/bathymetry/cg_smoother_base.cpp#L431) | **Partial match.** The reference-Hessian reuse *is* the affine optimization; the data-fitting basis evaluations are not hoisted (see B2). |
...
2. **(Perf, low) Hoist the reference basis tables.** `basis().evaluate(u,v)` at the fixed
   ...
   [cg_smoother_base.cpp:336-364](../../src/bathymetry/cg_smoother_base.cpp#L336)
   (Paper 2 B2/B4).
