...
TEST_F(CGLinearBezierSmootherTest, TwoByTwoMeshLinear) {
    ...
    EXPECT_GT(actual_range, 0.5 * expected_range);  // At least 50% of expected slope
    EXPECT_LT(actual_range, 1.5 * expected_range);  // At most 150% of expected slope
}
...
TEST_F(CGLinearBezierVTKTest, VTKOutputUniformMesh) {
    ...
    smoother.write_vtk(filename, 4);  // 4 subdivisions per element
...
TEST_F(CGLinearBezierSmootherTest, KattegatGeoTiffIntegration) {
    ...
    smoother.write_vtk(output_base, 4);
