...
int AreaOfInterest::define_region(const std::string &name,
  ...
  if (region_bits_.count(name)) {
    throw std::runtime_error("AreaOfInterest: Region '" + name +
                             "' already defined");
  }
...
int AreaOfInterest::define_region(
  ...
  if (polygon_points.size() < 3) {
    throw std::runtime_error(
        "AreaOfInterest: Polygon must have at least 3 points");
  }
  if (region_bits_.count(name)) {
    throw std::runtime_error("AreaOfInterest: Region '" + name +
                             "' already defined");
...
void AreaOfInterest::remove_region(const std::string &name) {
  ...
  regions.erase(std::remove_if(regions.begin(), regions.end(),
                               [bit_to_remove](const AOIRegion &r) {
                                 return r.bit_index == bit_to_remove;
                               }),
                regions.end());
...
AOIFlags
AreaOfInterest::compute_element_flags(const ElementBounds &bounds) const {
  AOIFlags flags = AOI_NONE;

  // Element center (2D)
  Point2D center(0.5 * (bounds.xmin + bounds.xmax),
                 0.5 * (bounds.ymin + bounds.ymax));

  // Element bounding box (for intersection test)
  Box2D elem_box(Point2D(bounds.xmin, bounds.ymin),
                 Point2D(bounds.xmax, bounds.ymax));
...
void AreaOfInterest::on_refine(const OctreeAdapter &mesh,
...
void AreaOfInterest::on_coarsen(const std::vector<Index> &children,
                                Index parent_elem) {
...
void AreaOfInterest::resize(Index num_elements) {
  flags_.resize(num_elements, AOI_NONE);
}
...
bool AreaOfInterest::is_in_region(Index elem,
...
std::vector<Index>
AreaOfInterest::elements_in_region(const std::string &region_name) const {
...
std::vector<Real>
AreaOfInterest::as_cell_data(const std::string &region_name) const {
...
int AreaOfInterest::num_regions() const {
