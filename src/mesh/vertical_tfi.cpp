...
VerticalTFI::VerticalTFI(const HexahedronBasis &basis, SigmaStretchType stretch_type,
                         const SigmaStretchParams &stretch_params)
    : basis_(basis), stretch_type_(stretch_type), stretch_params_(stretch_params),
      n_vert_(basis.order() + 1), n_horiz_((basis.order() + 1) * (basis.order() + 1)) {
    ...
    for (int k = 0; k < n_vert_; ++k) {
        ...
        // Apply stretching
        sigma_at_lgl_(k) =
            SigmaCoordinate::apply_stretching(sigma_uniform, stretch_type_, stretch_params_);
    }
}
...
void VerticalTFI::update_z_coordinates(const VecX &eta, const VecX &h, VecX &z) const {
    if (static_cast<int>(eta.size()) != n_horiz_) {
        throw std::invalid_argument("eta size mismatch");
    }
...
void VerticalTFI::compute_node_positions(const VecX &x_horiz, const VecX &y_horiz, const VecX &eta,
...
void VerticalTFI::compute_mesh_velocity(const VecX &deta_dt, const VecX &sigma_values,
                                        VecX &w_mesh) const {
...
void VerticalTFI::compute_mesh_velocity_full(const VecX &deta_dt, const VecX &dh_dt,
                                             const VecX &eta, const VecX &h, VecX &w_mesh) const {
    const int n_total = n_horiz_ * n_vert_;
    w_mesh.resize(n_total);
...
void VerticalTFI::compute_jacobians(const std::vector<Vec3> &nodes, std::vector<Mat3> &jacobians,
                                    VecX &det_J) const {
...
void VerticalTFI::compute_dz_dx(const VecX &deta_dx, const VecX &dh_dx, VecX &dz_dx) const {
    const int n_total = n_horiz_ * n_vert_;
    dz_dx.resize(n_total);

    for (int i = 0; i < n_horiz_; ++i) {
        // dz/dx|_sigma = deta/dx + sigma * dH/dx = deta/dx + sigma*(deta/dx +
        // dh/dx)
...
void VerticalTFI::compute_dz_dy(const VecX &deta_dy, const VecX &dh_dy, VecX &dz_dy) const {
    const int n_total = n_horiz_ * n_vert_;
...
GeometricFactors GeometricFactors::compute(const HexahedronBasis &basis, int num_quad_1d,
