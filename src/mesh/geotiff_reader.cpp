...
BathymetryData GeoTiffReader::load(const std::string& filename) {
    ...
    // Also auto-detect based on data statistics: if min > 0, likely depth format
    ...
    if (data.xmin > data.xmax) std::swap(data.xmin, data.xmax);
    if (data.ymin > data.ymax) std::swap(data.ymin, data.ymax);

#else
    error_message_ = "GDAL support not available. Rebuild with -DDRIFTER_USE_GDAL=ON";
#endif
...
std::function<bool(const ElementBounds&)> BathymetryMeshGenerator::create_refinement_function() const {
