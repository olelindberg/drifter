...
// =============================================================================
// C¹ vertex derivative constraints
// =============================================================================

void CGCubicBezierDofManager::build_vertex_derivative_constraints() {
    vertex_derivative_constraints_.clear();

    // C¹ derivative orders: z_u, z_v, z_uv
    // (z value is already shared via CG DOFs)
    static const std::vector<std::pair<int, int>> deriv_orders = {
        {1, 0}, {0, 1}, {1, 1}
    };

    // Build map from vertex position to (element, corner) pairs
    ...
    // Create constraints at shared vertices (star pattern)
    for (const auto& [key, elem_corners] : vertex_map) {
        ...
        for (size_t i = 1; i < elem_corners.size(); ++i) {
            ...
            for (const auto& [nu, nv] : deriv_orders) {
                CubicVertexDerivativeConstraint c;
                c.elem1 = ref_elem;
                c.elem2 = other_elem;
    ...
    mesh_.for_each_interior_edge([&](Index fine_elem, int fine_edge,
        ...
        bool at_end = (info.subedge_index == 0);
        switch (fine_edge) {
            case 0: tjunction_corner = at_end ? 2 : 0; break;
            case 1: tjunction_corner = at_end ? 3 : 1; break;
            case 2: tjunction_corner = at_end ? 1 : 0; break;
            case 3: tjunction_corner = at_end ? 3 : 2; break;
            default: return;
        }
