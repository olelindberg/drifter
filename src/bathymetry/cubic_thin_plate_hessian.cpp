...
CubicThinPlateHessian::CubicThinPlateHessian(int ngauss, Real gradient_weight)
    : ngauss_(ngauss)
...
VecX CubicThinPlateHessian::gradient(const VecX& coeffs) const {
    if (coeffs.size() != CubicBezierBasis2D::NDOF) {
        throw std::invalid_argument("CubicThinPlateHessian::gradient: coeffs must have 16 elements");
    }
