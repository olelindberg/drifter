...
void BezierMultigridPreconditioner::setup(
    ...
    // With tree-based coarsening (recursive prolongation), we can coarsen all the way
    // to min_tree_level regardless of min_depth, since we traverse the tree structure
    ...
    if (config_.verbose) {
        ...
        std::cerr << "[Multigrid] Building " << num_levels << " levels (composite grid)\n";
    }
    ...
    int finest_level = num_levels - 1;
    {
        ...
        levels_[finest_level].diag_inv = VecX(levels_[finest_level].num_dofs);
        for (Index i = 0; i < levels_[finest_level].num_dofs; ++i) {
            Real diag = Q_fine.coeff(i, i);
            levels_[finest_level].diag_inv(i) = (std::abs(diag) > 1e-14) ? 1.0 / diag : 0.0;
        }
    }

    if (config_.verbose) {
        std::cerr << "[Multigrid] Level " << finest_level << " (finest): "
                  << levels_[finest_level].num_dofs << " DOFs\n";
    }
    ...
    if (config_.smoother_type == SmootherType::MultiplicativeSchwarz ||
        ...
        OptionalScopedTimer t(profile_ ? &profile_->setup_element_blocks_ms : nullptr);
        build_element_blocks(finest_level, dof_manager);

        if (config_.smoother_type == SmootherType::ColoredMultiplicativeSchwarz) {
            build_element_coloring(finest_level);
        }
    }
    ...
    // Each MG level L has operators: R maps L+1 -> L (restriction), P maps L -> L+1 (prolongation)
    for (int mg_level = finest_level - 1; mg_level >= 0; --mg_level) {
        OptionalScopedTimer t(profile_ ? &profile_->setup_prolongation_ms : nullptr);
        ...
        levels_[mg_level].diag_inv = VecX(coarse_num_dofs);
        for (Index i = 0; i < coarse_num_dofs; ++i) {
            Real diag = levels_[mg_level].Q.coeff(i, i);
            levels_[mg_level].diag_inv(i) = (std::abs(diag) > 1e-14) ? 1.0 / diag : 0.0;
        }
        ...
        if (config_.verbose) {
            std::cerr << "[Multigrid] Level " << mg_level << " max_node_level=" << max_node_level << "\n";
        }

        if (max_node_level <= config_.min_tree_level) {
            // This is the coarsest level we want - truncate any remaining levels below
            if (mg_level > 0) {
                ...
                if (config_.verbose) {
                    std::cerr << "[Multigrid] Reached min_tree_level=" << config_.min_tree_level
                              << ", keeping " << levels_.size() << " levels\n";
                }
            }
            break;
        }
...
SpMat BezierMultigridPreconditioner::build_prolongation_for_level(
    ...
    // If no coarse nodes have children at the fine level, coarsening is not possible
    ...
    auto get_child_index = [](const QuadtreeNode* child, const QuadtreeNode* parent) -> int {
...
void BezierMultigridPreconditioner::v_cycle(int level, VecX &x,
    ...
    if (level == 0) {
        OptionalScopedTimer t(profile_ ? &profile_->vcycle_coarse_solve_ms : nullptr);
        if (profile_) {
    ...
    {
        OptionalScopedTimer t(profile_ ? &profile_->vcycle_post_smooth_ms : nullptr);
        if ((config_.smoother_type == SmootherType::MultiplicativeSchwarz ||
             config_.smoother_type == SmootherType::AdditiveSchwarz ||
             config_.smoother_type == SmootherType::ColoredMultiplicativeSchwarz) &&
            level == finest_level) {
...
int BezierMultigridPreconditioner::compute_min_leaf_depth(const QuadtreeAdapter &mesh) {
...
int BezierMultigridPreconditioner::get_child_quadrant(const QuadtreeNode *child,
...
void BezierMultigridPreconditioner::build_composite_hierarchy(
    ...
    for (const QuadtreeNode* leaf : mesh.elements()) {
        ...
        finest_grid.max_depth = std::max(finest_grid.max_depth, leaf->level.max_level());
        finest_grid.node_index_map[leaf] = static_cast<Index>(finest_grid.nodes.size());
    ...
    if (config_.verbose) {
        std::cerr << "[Multigrid] Composite level " << finest_level << ": "
                  << finest_grid.nodes.size() << " nodes (0 subdivision, 0 pass-through), "
                  << finest_grid.num_dofs << " DOFs, max_depth=" << finest_grid.max_depth << "\n";
    }
...
void BezierMultigridPreconditioner::build_composite_level_from_finer(int mg_level) {
    ...
    // Level 0 = min_tree_level, Level 1 = min_tree_level + 1, etc.
    int target_depth = config_.min_tree_level + mg_level;
    ...
    for (const QuadtreeNode* tree_node : nodes_at_depth) {
        ...
        if (fine_grid_nodes.count(tree_node) > 0) {
            ...
            coarse_grid.max_depth = std::max(coarse_grid.max_depth, tree_node->level.max_level());
            coarse_grid.node_index_map[tree_node] = static_cast<Index>(coarse_grid.nodes.size());
            ...
            assign_dofs_from_bounds(node, tree_node->bounds, position_to_dof, coarse_grid.num_dofs);
    ...
    if (config_.verbose) {
        std::cerr << "[Multigrid] Composite level " << mg_level << ": "
                  ...
                  << levels_[mg_level].subdivision_node_indices.size() << " subdivision, "
                  << levels_[mg_level].passthrough_node_indices.size() << " pass-through), "
                  << coarse_grid.num_dofs << " DOFs, max_depth=" << coarse_grid.max_depth << "\n";
    }
}
...
void BezierMultigridPreconditioner::add_recursive_prolongation(
    ...
    // Check if this tree node is in the fine grid (i.e., it's a leaf at fine level)
    ...
    if (it != fine_grid.node_index_map.end()) {
        ...
        for (int fine_local = 0; fine_local < 16; ++fine_local) {
            ...
            // Skip if this fine DOF has already been processed by another coarse element
            // (happens at shared edges/vertices between coarse elements)
            if (processed_fine_dofs.count(fine_dof) > 0) continue;
            processed_fine_dofs.insert(fine_dof);
    ...
    for (const auto &child : tree_node->children) {
        ...
        add_recursive_prolongation(child.get(), coarse_node, fine_grid, P_new,
                                   P_local, triplets, processed_fine_dofs, tree_node);
    }
}

SpMat BezierMultigridPreconditioner::build_prolongation_composite(int mg_level) {
    ...
    // Track which fine DOFs have been processed to avoid duplicate contributions
    // from multiple coarse elements at shared boundaries
    std::set<Index> processed_fine_dofs;

    // Identity matrix for starting recursive subdivision
    ...
    for (Index coarse_idx : levels_[mg_level].subdivision_node_indices) {
        ...
        for (const auto &tree_child : parent->children) {
            ...
            add_recursive_prolongation(tree_child.get(), coarse_node, fine_grid,
                                       P_child, P_local, triplets, processed_fine_dofs, parent);
        }
    }
    ...
    for (Index coarse_idx : levels_[mg_level].passthrough_node_indices) {
        ...
        if (it == fine_grid.node_index_map.end()) continue;
        ...
        for (int local = 0; local < 16; ++local) {
            ...
            if (fine_dof >= 0 && coarse_dof >= 0) {
                // Skip if already processed by subdivision or another pass-through
                if (processed_fine_dofs.count(fine_dof) > 0) continue;
                processed_fine_dofs.insert(fine_dof);
    ...
    if (config_.verbose) {
        ...
        std::cerr << "[Multigrid] Prolongation " << mg_level << "->" << (mg_level + 1)
                  << ": " << P.rows() << "x" << P.cols() << ", "
                  << identity_rows << " identity rows, "
                  << subdivision_rows << " subdivision rows\n";
    }
...
SpMat BezierMultigridPreconditioner::build_restriction_l2(int mg_level) {
    ...
    // Bezier subdivision matrices for each child quadrant (for recursive traversal)
    std::array<MatX, 4> P_local;
    P_local[0] = kronecker_product(S_left_, S_left_);   // (0,0)
    P_local[1] = kronecker_product(S_right_, S_left_);  // (1,0)
    P_local[2] = kronecker_product(S_left_, S_right_);  // (0,1)
    ...
    // Track which (coarse_dof, fine_dof) pairs have been processed to avoid duplicates
    ...
    for (Index coarse_idx : levels_[mg_level].subdivision_node_indices) {
        ...
        for (const auto &tree_child : parent->children) {
            int child_quadrant = get_child_quadrant(tree_child.get(), parent);
            MatX P_accumulated = P_local[child_quadrant];  // Start with single subdivision
            int depth = 1;  // Depth from coarse node

            // Lambda to recursively process descendants
            std::function<void(const QuadtreeNode*, const MatX&, int)> process_descendants =
                [&](const QuadtreeNode* node, const MatX& P_acc, int d) {
                    ...
                    if (it != fine_grid.node_index_map.end()) {
                        ...
                        for (int coarse_local = 0; coarse_local < 16; ++coarse_local) {
                            ...
                            for (int fine_local = 0; fine_local < 16; ++fine_local) {
                                ...
                                if (processed_pairs.count(pair) > 0) continue;
    ...
    if (config_.verbose) {
        std::cerr << "[Multigrid] L2 Restriction " << (mg_level + 1) << "->" << mg_level
                  << ": " << R.rows() << "x" << R.cols() << ", nnz=" << R.nonZeros() << "\n";
    }
