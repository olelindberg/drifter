...
VecX LinearBezierBasis2D::evaluate_bernstein_derivative_1d(int n, Real t, int k) const {
    if (k == 0) {
...
Real LinearBezierBasis2D::evaluate_scalar(const VecX &coeffs, Real u, Real v) const {
...
Vec2 LinearBezierBasis2D::corner_param(int corner_id) const {
    ...
    case 3:
        return Vec2(1.0, 1.0);
    default:
        throw std::invalid_argument("Invalid corner_id: " + std::to_string(corner_id));
    }
}
