...
void write_error_csv(const std::string &dir, int iteration,
                     ...
                     const std::vector<Index> &marked_elements,
  ...
  fname << dir << "/errors_iter_" << std::setw(3) << std::setfill('0')
        << iteration << ".csv";
  ...
  ofs << "element_id,center_x,center_y,l2_error,max_error,normalized_error,"
         ...
         "gradient_indicator,curvature_indicator,weno_indicator,marked\n";
  ...
  for (const auto &err : errors) {
    ...
    ofs << err.element << "," << cx << "," << cy << "," << err.l2_error << ","
        ...
        << "," << err.mean_depth << "," << err.relative_error << ","
        << err.gradient_indicator << "," << err.curvature_indicator << ","
        << err.weno_indicator << "," << (is_marked ? 1 : 0) << "\n";
  }
...
void AdaptiveCGLinearBezierSmoother::set_bathymetry_surface(
    const BathymetrySurface &surface) {
  // Set depth function
  bathy_func_ = [&surface](Real x, Real y) -> Real {
  ...
  curv_func_ = [&surface](Real x, Real y, Real &d2h_dx2, Real &d2h_dxdy,
                          Real &d2h_dy2) {
    surface.curvature(x, y, d2h_dx2, d2h_dxdy, d2h_dy2);
  };

  // Set land mask function
  land_mask_func_ = [&surface](Real x, Real y) -> bool {
...
void AdaptiveCGLinearBezierSmoother::compute_element_error_statistics(
    Index elem, Real &l2_error, Real &mean_error, Real &std_error,
...
AdaptiveCGLinearBezierSmoother::estimate_element_error(Index elem) const {
  ...
  compute_element_error_statistics(elem, result.l2_error, result.mean_error,
                                   result.std_error, result.mean_depth);
  ...
  result.relative_error = result.normalized_error /
                          (std::abs(result.mean_depth) + config_.depth_scale);
  ...
  result.weno_indicator =
      config_.weno_gradient_weight * result.gradient_indicator +
...
Real AdaptiveCGLinearBezierSmoother::compute_gradient_indicator(
    Index elem) const {
  if (!grad_func_)
    return 0.0;
  ...
  Real dx = bounds.xmax - bounds.xmin;
  Real dy = bounds.ymax - bounds.ymin;
  Real h = std::sqrt(dx * dy); // Characteristic element size
  int ngauss = config_.ngauss_error;
  ...
  // WENO scaling: h² × |∇z|² (dimensionally [length]⁴)
  return h * h * mean_grad_sq;
}

Real AdaptiveCGLinearBezierSmoother::compute_curvature_indicator(
    Index elem) const {
  if (!curv_func_)
    return 0.0;
  ...
  for (int j = 0; j < ngauss; ++j) {
    for (int i = 0; i < ngauss; ++i) {
      ...
      Real hess_sq =
          d2h_dx2 * d2h_dx2 + 2.0 * d2h_dxdy * d2h_dxdy + d2h_dy2 * d2h_dy2;
  ...
  // WENO scaling: h⁴ × |H|²_F (dimensionally [length]⁴)
  return h * h * h * h * mean_curv_sq;
}
...
CGLinearAdaptationResult AdaptiveCGLinearBezierSmoother::adapt_once() {
  ...
  if (config_.verbose &&
      config_.error_metric_type == ErrorMetricType::RelativeError &&
      max_err > 1.0) {
    ...
    std::cout << "  Bounds: [" << bounds.xmin << ", " << bounds.xmax << "] x ["
    ...
    std::cout << "  Formula: " << worst.normalized_error << " / ("
              << std::abs(worst.mean_depth) << " + " << config_.depth_scale
  ...
  if (!config_.error_output_dir.empty()) {
    write_error_csv(config_.error_output_dir, result.iteration, errors,
                    selected, *quadtree_);
  }
...
void AdaptiveCGLinearBezierSmoother::write_vtk(const std::string &filename,
  ...
  io::write_cg_bezier_surface_vtk(
      filename, *quadtree_,
      [this](Real x, Real y) { return evaluate(x, y); }, dof_mgr.xmin_domain(),
      dof_mgr.ymin_domain(), dof_mgr.inv_quantization_tol(),
