...
AdaptiveBezierSmoother::AdaptiveBezierSmoother(Real xmin, Real xmax,
                                               Real ymin, Real ymax,
                                               int nx, int ny,
                                               const AdaptiveBezierConfig& config)
    : config_(config) {
...
AdaptiveBezierSmoother::AdaptiveBezierSmoother(OctreeAdapter& octree,
                                               const AdaptiveBezierConfig& config)
    : config_(config)
    , octree_(&octree) {
...
void AdaptiveBezierSmoother::init_gauss_quadrature() {
  ...
  if (ngauss == 1) {
    ...
    gauss_nodes_ << 0.0469100770306680, 0.2307653449471585,
                    0.5, 0.7692346550528415, 0.9530899229693319;
    ...
    throw std::invalid_argument(
        "AdaptiveBezierSmoother: ngauss_error must be between 1 and 6");
  }
}
...
void AdaptiveBezierSmoother::set_bathymetry_data(const BathymetrySource& source) {
...
void AdaptiveBezierSmoother::set_bathymetry_data(
...
void AdaptiveBezierSmoother::rebuild_smoother() {
  // Sync quadtree with refined octree
  quadtree_ = std::make_unique<QuadtreeAdapter>(*octree_);

  // Create new smoother for updated mesh
  smoother_ = std::make_unique<BezierBathymetrySmoother>(
...
void AdaptiveBezierSmoother::apply_bathymetry_to_smoother() {
  if (!smoother_) {
    throw std::runtime_error(
        "AdaptiveBezierSmoother: smoother not initialized");
  }
  if (!bathy_func_) {
    throw std::runtime_error(
        "AdaptiveBezierSmoother: bathymetry data not set");
  }
...
Real AdaptiveBezierSmoother::compute_element_l2_error(Index elem) const {
  if (!smoother_ || !smoother_->is_solved()) {
    throw std::runtime_error(
        "AdaptiveBezierSmoother: must solve before computing errors");
  }
...
ElementErrorEstimate AdaptiveBezierSmoother::estimate_element_error(
    Index elem) const {
  ElementErrorEstimate result;
  result.element = elem;
...
std::vector<ElementErrorEstimate> AdaptiveBezierSmoother::estimate_errors() const {
  std::vector<ElementErrorEstimate> errors;
...
Real AdaptiveBezierSmoother::max_error() const {
...
Real AdaptiveBezierSmoother::mean_error() const {
  auto errors = estimate_errors();
  Real sum_err = 0.0;
  for (const auto& err : errors) {
...
void AdaptiveBezierSmoother::refine_elements(
...
AdaptationResult AdaptiveBezierSmoother::adapt_once() {
  ...
  std::sort(errors.begin(), errors.end(),
            [](const ElementErrorEstimate& a, const ElementErrorEstimate& b) {
...
AdaptationResult AdaptiveBezierSmoother::solve_adaptive() {
  if (!bathy_func_) {
    throw std::runtime_error(
        "AdaptiveBezierSmoother: bathymetry data not set");
  }
...
const BezierBathymetrySmoother& AdaptiveBezierSmoother::smoother() const {
  if (!smoother_) {
    throw std::runtime_error(
        "AdaptiveBezierSmoother: smoother not initialized (call solve first)");
  }
  return *smoother_;
}

Real AdaptiveBezierSmoother::evaluate(Real x, Real y) const {
  if (!smoother_ || !smoother_->is_solved()) {
    throw std::runtime_error(
        "AdaptiveBezierSmoother: must solve before evaluating");
  }
  return smoother_->evaluate(x, y);
}

void AdaptiveBezierSmoother::write_vtk(const std::string& filename,
                                       int resolution) const {
  if (!smoother_ || !smoother_->is_solved()) {
    throw std::runtime_error(
        "AdaptiveBezierSmoother: must solve before writing VTK");
  }
