...
void CGBezierDofManager::assign_edge_dofs_nonconforming() {
    // For non-conforming (FineToCoarse) edges, only CORNER DOFs should be
    // shared with the coarse element.
    //
    // IMPORTANT: Unlike conforming edges where DOFs at the same physical
    // position can be shared, at non-conforming edges only the curve ENDPOINTS
    // are guaranteed to match. Interior edge DOFs (Bezier control points) do
    // NOT lie on the curve and have values that depend on the curve shape, not
    // just physical position.
    //
    // assign_edge_dofs() may have incorrectly shared interior DOFs based on
    // physical position matching. We must UNDO this by assigning NEW global DOF
    // indices to interior fine edge DOFs that were incorrectly shared with the
    // coarse element.
    //
    ...
    // - T-junction (k=5 for subedge=0, k=0 for subedge=1): shares with adjacent
    // fine element
    //
    ...
    // First pass: identify which coarse DOFs should NOT be shared with fine
    // elements (all interior DOFs on the coarse side of non-conforming edges)
    ...
    for (Index elem = 0; elem < mesh_.num_elements(); ++elem) {
        for (int edge = 0; edge < 4; ++edge) {
            ...
            // Determine which DOF is the T-junction (shared with adjacent fine
            // element)
            ...
            for (int k = 0; k < static_cast<int>(fine_dofs.size()); ++k) {
                ...
                if (k == shared_k) {
                    ...
                    // position matching) Don't change this DOF
                } else {
                    // Interior DOF: check if it was incorrectly shared with a
                    // coarse DOF
                    if (coarse_interior_dofs.count(current_global) > 0) {
                        // This DOF was incorrectly shared with a coarse
                        // interior DOF Assign a NEW global DOF index
                        elem_to_global_[elem][fine_local] = num_global_dofs_++;
                    }
                    // If it wasn't shared with coarse, it might be shared with
                    // another fine element (conforming edge), which is correct
                    // - leave it alone
                }
            }
        }
...
void CGBezierDofManager::build_hanging_node_constraints() {
    ...
    // NOTE: We do NOT use for_each_interior_edge() because its "lower index
    // owns" filtering causes FineToCoarse edges to be skipped when the coarse
    // element has a lower index than the fine element (common after AMR
    // refinement).
    for (Index elem = 0; elem < mesh_.num_elements(); ++elem) {
        for (int edge = 0; edge < 4; ++edge) {
            EdgeNeighborInfo info = mesh_.get_neighbor(elem, edge);
            ...
            // For each fine edge DOF, create a constraint if it's a hanging
            // node
            for (int k = 0; k < static_cast<int>(fine_dofs.size()); ++k) {
                int fine_local = fine_dofs[k];
                Index fine_global = elem_to_global_[elem][fine_local];

                // Only CORNER DOFs are shared (curve endpoints where values
                // must match). All other fine edge DOFs must be CONSTRAINED
                // using the extraction matrix. This is because Bezier control
                // points don't lie on the curve (except at endpoints), so
                // sharing control points at the same physical position does NOT
                // ensure C⁰ continuity of the curve.
                //
                ...
                if (is_shared) {
                    // This DOF is shared (handled by
                    // assign_edge_dofs_nonconforming)
                    continue;
                }

                // Skip if this DOF is already constrained (avoid duplicate
                // constraints at T-junctions where multiple fine elements meet
                // the same coarse edge)
                if (constrained_dofs_.count(fine_global) > 0) {
                    continue;
                }

                // This is a hanging node - constrain it using de Casteljau
                // subdivision weights
...
Vec2 get_edge_param(int edge, Real t) {
    switch (edge) {
    case 0:
        return Vec2(0.0, t); // Left edge: u=0, v varies
    case 1:
        return Vec2(1.0, t); // Right edge: u=1, v varies
    case 2:
        return Vec2(t, 0.0); // Bottom edge: v=0, u varies
    case 3:
        return Vec2(t, 1.0); // Top edge: v=1, u varies
    default:
        return Vec2(0.5, 0.5);
    }
}
