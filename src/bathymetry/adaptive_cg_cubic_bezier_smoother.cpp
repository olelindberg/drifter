...
AdaptiveCGCubicBezierSmoother::AdaptiveCGCubicBezierSmoother(Real xmin, Real xmax,
                                                               Real ymin, Real ymax,
                                                               int nx, int ny,
...
AdaptiveCGCubicBezierSmoother::AdaptiveCGCubicBezierSmoother(OctreeAdapter& octree,
                                                               const AdaptiveCGCubicBezierConfig& config)
    : config_(config)
    , octree_(&octree) {
...
void AdaptiveCGCubicBezierSmoother::init_gauss_quadrature() {
  ...
  if (ngauss == 1) {
    ...
    throw std::invalid_argument(
        "AdaptiveCGCubicBezierSmoother: ngauss_error must be between 1 and 6");
  }
}
...
Real AdaptiveCGCubicBezierSmoother::compute_element_l2_error(Index elem) const {
  if (!smoother_ || !smoother_->is_solved()) {
    throw std::runtime_error(
        "AdaptiveCGCubicBezierSmoother: must solve before computing errors");
  }
...
CGCubicElementErrorEstimate AdaptiveCGCubicBezierSmoother::estimate_element_error(
    Index elem) const {
...
const CGCubicBezierBathymetrySmoother& AdaptiveCGCubicBezierSmoother::smoother() const {
  if (!smoother_) {
    throw std::runtime_error(
        "AdaptiveCGCubicBezierSmoother: smoother not initialized (call solve first)");
  }
  return *smoother_;
}
...
void AdaptiveCGCubicBezierSmoother::write_vtk(const std::string& filename,
