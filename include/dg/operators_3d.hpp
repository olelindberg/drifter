...
public:
    ...
    explicit DG3DElementOperator(const HexahedronBasis& basis,
...
/// @details Given physical coordinates at nodes, compute Jacobian and metric terms
...
public:
    ...
    /// @param basis Hexahedron basis
    /// @param quad Volume quadrature
    SigmaCoordinateOperators(const HexahedronBasis& basis,
                              const GaussQuadrature3D& quad);
    ...
    /// @brief Update metrics when free surface changes (steady state, no time derivative)
    ...
    /// @details DU/Dt = dU/dt + u*dU/dx + v*dU/dy + (omega - w_mesh/H)*dU/dsigma
