...
///
/// For a Bezier surface z(u,v) = sum_k c_k * B_k(u,v), the thin plate energy is:
...
///   H = D2U^T * W * D2U + D2V^T * W * D2V + 2 * D2U^T * W * D2V + 2 * D2UV^T * W * D2UV
...
public:
    ...
    /// @param ngauss Number of Gauss points per direction (default 6 for quintic)
    ...
    ///   E_physical = (dx*dy) * integral[(1/dx^2 * z_uu + 1/dy^2 * z_vv)^2 + ...] du dv
    ///
