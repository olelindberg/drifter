...
///
/// For a Bezier surface z(u,v) = sum_k c_k * B_k(u,v), the triharmonic energy is:
///   E = integral_{[0,1]^2} |grad(Laplacian z)|^2 du dv
///     = integral [(z_uuu + z_uvv)^2 + (z_uuv + z_vvv)^2] du dv
///
/// This can be written as E = c^T * H * c where H is a symmetric positive semi-definite
/// matrix that depends only on the basis functions (not on the coefficients).
///
/// The Hessian is computed using Gauss-Legendre quadrature:
///   H = G1^T * W * G1 + G2^T * W * G2
/// where G1 = D3UUU + D3UVV and G2 = D3UUV + D3VVV are the gradient of Laplacian
/// evaluation matrices at Gauss points, and W is the diagonal matrix of quadrature weights.
class TriharmonicHessian {
public:
    /// @brief Construct triharmonic Hessian with specified quadrature order
    /// @param ngauss Number of Gauss points per direction (default 4, minimum 3)
    ...
    ///   etc.
    ///
    /// The energy integral scales as dx*dy (Jacobian), combined with derivative scaling.
    ///
