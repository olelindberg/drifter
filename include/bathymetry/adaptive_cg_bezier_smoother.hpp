...
/// @file adaptive_cg_bezier_smoother.hpp
/// @brief Adaptive mesh refinement for CG Bezier bathymetry smoothing
///
/// Implements coupled adaptive refinement + CG Bezier smoothing where the
/// Bezier solution error drives mesh refinement. The algorithm iteratively:
/// 1. Solves CG Bezier smoothing on current mesh
...
///
/// Uses the CG (Continuous Galerkin) Bezier smoother which shares DOFs at
...
struct AdaptiveCGBezierConfig {
  ...
  // Smoother configuration (passed to CGBezierBathymetrySmoother)
  CGBezierSmootherConfig smoother_config;
...
///
/// Iteratively solves the CG Bezier smoothing problem and refines elements
/// where the fitting error exceeds a threshold. Uses Continuous Galerkin
/// assembly where DOFs at element boundaries are shared, providing natural
/// smoothing across element boundaries.
///
/// @par Algorithm:
/// @code
/// REPEAT:
...
///
/// smoother.write_vtk("/tmp/adaptive_cg_bathymetry", 10);
/// @endcode
class AdaptiveCGBezierSmoother {
public:
  ...
  AdaptiveCGBezierSmoother(Real xmin, Real xmax, Real ymin, Real ymax,
                           int nx, int ny,
  ...
  CGAdaptationResult solve_adaptive();

  /// @brief Perform single adaptation iteration
  /// @return Result of this iteration
  CGAdaptationResult adapt_once();

  /// @brief Get adaptation history (all iterations)
  const std::vector<CGAdaptationResult>& history() const { return history_; }
  ...
  std::vector<CGElementErrorEstimate> estimate_errors() const;

  /// @brief Estimate error for single element
  /// @param elem Element index
  /// @return Error estimate for this element
  /// @pre Must have called solve_adaptive() or adapt_once() at least once
  CGElementErrorEstimate estimate_element_error(Index elem) const;
  ...
  const CGBezierBathymetrySmoother& smoother() const;
...
private:
  ...
  std::unique_ptr<CGBezierBathymetrySmoother> smoother_;

  // Adaptation history
  std::vector<CGAdaptationResult> history_;
