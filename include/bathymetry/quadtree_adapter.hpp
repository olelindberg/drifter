...
struct QuadBounds {
  ...
  bool contains(const Vec2 &p, Real tol) const {
    return p(0) >= xmin - tol && p(0) <= xmax + tol && p(1) >= ymin - tol &&
           p(1) <= ymax + tol;
  }
};
...
public:
  ...
  void build_uniform(Real xmin, Real xmax, Real ymin, Real ymax, int nx,
                     int ny);
  ...
  template <typename Func> void for_each_interior_edge(Func &&f) const {
  ...
  template <typename Func> void for_each_boundary_edge(Func &&f) const {
    for (Index e = 0; e < num_elements(); ++e) {
      for (int edge = 0; edge < 4; ++edge) {
        EdgeNeighborInfo info = get_neighbor(e, edge);
        if (info.is_boundary()) {
...
private:
  ...
  std::vector<QuadtreeNode *> find_fine_neighbors_at_edge(QuadtreeNode *node,
                                                          int edge_id) const;

  /// Create a node from octree element bounds
  std::unique_ptr<QuadtreeNode>
  create_node_from_octree(const ElementBounds &bounds3d, Index octree_idx);
};
