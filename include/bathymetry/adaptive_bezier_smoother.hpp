...
/// @file adaptive_bezier_smoother.hpp
/// @brief Adaptive mesh refinement for Bezier bathymetry smoothing
///
/// Implements coupled adaptive refinement + Bezier smoothing where the
/// Bezier solution error drives mesh refinement. The algorithm iteratively:
/// 1. Solves Bezier smoothing on current mesh
...
/// @brief Configuration for adaptive Bezier smoother
struct AdaptiveBezierConfig {
  // Stopping criteria
  ...
  // Smoother configuration (passed to BezierBathymetrySmoother)
  BezierSmootherConfig smoother_config;
...
/// @brief Adaptive Bezier bathymetry smoother with error-driven refinement
///
/// Iteratively solves the Bezier smoothing problem and refines elements
/// where the fitting error exceeds a threshold. Maintains C² continuity
/// across all levels of refinement via the underlying BezierBathymetrySmoother.
///
/// @par Algorithm:
/// @code
/// REPEAT:
...
///
/// smoother.write_vtk("/tmp/adaptive_bathymetry", 10);
/// @endcode
class AdaptiveBezierSmoother {
public:
  ...
  AdaptiveBezierSmoother(Real xmin, Real xmax, Real ymin, Real ymax,
                         int nx, int ny,
  ...
  explicit AdaptiveBezierSmoother(OctreeAdapter& octree,
                                  const AdaptiveBezierConfig& config = {});
  ...
  AdaptationResult solve_adaptive();

  /// @brief Perform single adaptation iteration
  /// @return Result of this iteration
  AdaptationResult adapt_once();

  /// @brief Get adaptation history (all iterations)
  const std::vector<AdaptationResult>& history() const { return history_; }
  ...
  std::vector<ElementErrorEstimate> estimate_errors() const;

  /// @brief Estimate error for single element
  /// @param elem Element index
  /// @return Error estimate for this element
  /// @pre Must have called solve_adaptive() or adapt_once() at least once
  ElementErrorEstimate estimate_element_error(Index elem) const;
  ...
  const BezierBathymetrySmoother& smoother() const;
...
private:
  ...
  std::unique_ptr<BezierBathymetrySmoother> smoother_;

  // Adaptation history
  std::vector<AdaptationResult> history_;
