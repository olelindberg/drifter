...
struct MultigridProfile {
  ...
  // Smoother timing (accumulated)
  double smoother_total_ms = 0.0; ///< Total smoother time (all methods)
...
struct MultigridLevel {
  ...
  /// Smoother for this level (levels > 0 only; level 0 uses direct solver)
  std::unique_ptr<IIterativeMethod> smoother;

  /// Element block LU factorizations for Schwarz smoothers
...
public:
  ...
  /// @param cache Pointer to map owned by adaptive smoother (persists across
  /// refinement)
...
private:
  ...
  /// @brief Compute element matrix for a subdivision node via local Galerkin
  /// @param node Subdivision node (has children, not a leaf)
  /// @return 16x16 element matrix computed as sum of P_child^T * Q_child *
  /// P_child
  MatX compute_subdivision_element_matrix(const QuadtreeNode *node) const;

  /// @brief Get element matrix recursively (cached or computed)
  /// @param node Quadtree node
  /// @return 16x16 element matrix (from cache if leaf, computed if subdivision)
  MatX get_element_matrix_recursive(const QuadtreeNode *node) const;
  ...
  /// @details Uses composite_grid.nodes[i].dof_indices for DOF mapping
