...
/// @details Represents a single cell in the adaptive octree with directional refinement
struct OctreeNode {
...
public:
    ...
    OctreeAdapter(Real xmin, Real xmax,
                  Real ymin, Real ymax,
                  Real zmin, Real zmax);
    ...
    void refine(const std::vector<Index>& elements,
    ...
    /// @brief Get elements in Morton order (for space-filling curve partitioning)
...
private:
    ...
    OctreeNode* find_neighbor_same_or_coarser(
        OctreeNode* node, int face_id) const;

    /// Find all fine neighbors at a face
    std::vector<OctreeNode*> find_fine_neighbors(
        OctreeNode* node, int face_id) const;
};
...
struct GradientCriterion {
    ...
    bool should_refine(const VecX& solution,
                       const ElementBounds& bounds,
                       const DirectionalLevel& level) const;

    RefineMask refinement_mask(const VecX& solution,
...
struct BathymetryCriterion {
    ...
    bool should_refine(const ElementBounds& bounds,
                       const DirectionalLevel& level) const;
};
