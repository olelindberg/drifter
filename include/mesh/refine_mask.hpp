...
inline int refinement_count(RefineMask mask) {
    int count = 0;
    if (refines_x(mask)) ++count;
    if (refines_y(mask)) ++count;
    if (refines_z(mask)) ++count;
    return count;
}

/// Number of children when refining with this mask
inline int num_children(RefineMask mask) {
    return 1 << refinement_count(mask);
}

/// Convert mask to string representation
inline std::string to_string(RefineMask mask) {
    if (mask == RefineMask::NONE) return "NONE";
    std::string s;
    if (refines_x(mask)) s += "X";
    if (refines_y(mask)) s += "Y";
    if (refines_z(mask)) s += "Z";
    return s;
}
...
struct DirectionalLevel {
    ...
    int max_level() const {
        return std::max({level_x, level_y, level_z});
    }

    /// Minimum level across all axes
    int min_level() const {
        return std::min({level_x, level_y, level_z});
    }
    ...
    RefineMask mask_to_reach(const DirectionalLevel& target) const {
        ...
        if (level_x < target.level_x) mask |= RefineMask::X;
        if (level_y < target.level_y) mask |= RefineMask::Y;
        if (level_z < target.level_z) mask |= RefineMask::Z;
        return mask;
    }

    bool operator==(const DirectionalLevel& other) const {
        return level_x == other.level_x &&
               level_y == other.level_y &&
