...
public:
    ...
    /// @param method Interpolation method (Bernstein recommended for boundedness)
    ...
    /// converts to Bernstein coefficients, and applies non-conforming projection.
    /// @param bathy Bathymetry data (e.g., from GeoTIFF)
    void set_from_bathymetry(const BathymetryData& bathy);

    /// @brief Initialize from bathymetry with scale-dependent box filter smoothing
    /// Samples bathymetry at LGL nodes using local averaging with no pixel cap,
    /// allowing large smoothing radii for coarse elements.
    /// @param bathy Bathymetry data (e.g., from GeoTIFF)
    ...
    /// @brief Get seabed element index for a mesh element (-1 if not bottom layer)
    ...
    /// @brief Get coordinates for a seabed element (3*(order+1)^2 interleaved x,y,z)
