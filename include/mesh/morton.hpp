...
// Adapted from SeaMesh: /home/ole/Projects/SeaMesh/src/Morton3D.h and MortonUtil.h
...
private:
    /// Spread bits of a 21-bit integer into every third bit (for 3D interleaving)
...
public:
    ...
    static inline uint64_t refine(uint64_t code,
                                   int ix, int iy, int iz,
                                   bool refine_x, bool refine_y, bool refine_z) {
        if (refine_x) code = child_x(code, ix);
        if (refine_y) code = child_y(code, iy);
        if (refine_z) code = child_z(code, iz);
        return code;
    }

    /// Get standard (isotropic) parent code
    static inline uint64_t parent(uint64_t code) {
        return code >> 3;
    }
