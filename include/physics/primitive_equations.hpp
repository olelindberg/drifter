...
public:
    ...
    PrimitiveEquationsElement(const HexahedronBasis &basis, const GaussQuadrature3D &quad,
    ...
    void compute_momentum_rhs(const PrimitiveState &state, VecX &dHu_dt, VecX &dHv_dt) const;

    /// @brief Compute tracer RHS only
    void compute_tracer_rhs(const PrimitiveState &state, VecX &dHT_dt, VecX &dHS_dt) const;
    ...
    void momentum_advection(const PrimitiveState &state, VecX &adv_Hu, VecX &adv_Hv) const;

    /// @brief Coriolis terms
    void coriolis_terms(const PrimitiveState &state, VecX &cor_Hu, VecX &cor_Hv) const;

    /// @brief Barotropic pressure gradient (-gH*grad(eta))
    void barotropic_pressure_gradient(const PrimitiveState &state, VecX &pg_u, VecX &pg_v) const;

    /// @brief Horizontal diffusion
    void horizontal_diffusion(const VecX &field, Real nu, VecX &diff_x, VecX &diff_y) const;

    /// @brief Vertical diffusion in sigma coordinates
    void vertical_diffusion(const VecX &field, Real kappa, const VecX &H, VecX &diff) const;

    /// @brief Advection for tracer field
    void tracer_advection(const PrimitiveState &state, const VecX &HT, VecX &adv_HT) const;
...
public:
    ...
    void initialize(int num_elements, const std::vector<VecX> &bathymetry,
                    ...
                    const CoriolisParameter &coriolis, const std::vector<VecX> &y_positions);
    ...
    void apply_boundary_conditions(const std::vector<PrimitiveState> &states,
                                   std::vector<PrimitiveTendencies> &tendencies, Real time) const;
...
struct WindStress {
    ...
    void compute(const VecX &x, const VecX &y, Real time, VecX &tau_x, VecX &tau_y) const;
};
...
public:
    ...
    explicit BottomFriction(Real Cd = 2.5e-3, Type type = Type::Quadratic) : Cd_(Cd), type_(type) {}

    /// @brief Compute bottom stress
    void compute(const VecX &u_bot, const VecX &v_bot, const VecX &H, VecX &tau_x,
                 VecX &tau_y) const;
