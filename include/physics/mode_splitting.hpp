...
public:
    ...
    ModeSplittingElement(const HexahedronBasis& basis,
    ...
    void barotropic_eta_rhs(const BarotropicState& state,
                             VecX& deta_dt) const;
    ...
    void barotropic_rhs(const BarotropicState& state,
    ...
    void baroclinic_pressure_forcing(const VecX& rho, const VecX& H,
                                       VecX& forcing_x, VecX& forcing_y) const;

    /// @brief Compute bottom stress contribution to barotropic
    void bottom_stress_forcing(const VecX& u_bot, const VecX& v_bot,
                                Real Cd, VecX& tau_x, VecX& tau_y) const;

    /// @brief Compute wind stress contribution
    void wind_stress_forcing(const VecX& tau_wind_x, const VecX& tau_wind_y,
                              const VecX& H, VecX& forcing_x, VecX& forcing_y) const;
    ...
    void barotropic_step_predictor_corrector(Real dt,
                                               const VecX& forcing_x,
                                               const VecX& forcing_y,
                                               BarotropicState& state) const;
    ...
    /// @param forcing_x, forcing_y Baroclinic forcing (constant during subcycling)
    /// @param state Barotropic state (updated in place)
    /// @param eta_avg Time-averaged eta over subcycling (for 3D pressure gradient)
    void subcycle(Real dt_3d, int n_subcycles,
...
public:
    ...
    Real compute_barotropic_dt(const std::vector<BarotropicState>& states,
                                Real dx_min) const;

    /// @brief Compute baroclinic CFL time step
    Real compute_baroclinic_dt(const std::vector<PrimitiveState>& states,
                                 Real dx_min) const;
...
private:
    ...
    void subcycle_barotropic(Real dt_3d,
                              std::vector<BarotropicState>& baro_state,
                              std::vector<VecX>& eta_avg);

    /// @brief Update baroclinic state
    void update_baroclinic(Real dt_3d,
                            const std::vector<VecX>& eta_avg,
