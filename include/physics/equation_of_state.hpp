...
//
// Computes density as a function of temperature (T), salinity (S), and pressure (p).
// Several formulations are provided:
//
...
public:
    ...
    /// @param T Temperature [°C] (potential or in-situ depending on formulation)
...
inline void EquationOfState::density(const VecX& T, const VecX& S, const VecX& p,
                                      VecX& rho) const {
...
inline Real EquationOfState::density_unesco(Real T, Real S, Real p) const {
    ...
    Real A = 8.24493e-1 - 4.0899e-3 * T + 7.6438e-5 * T2
    ...
    // K_w (pure water)
    Real K_w = 19652.21 + 148.4206 * T - 2.327105 * T2
...
public:
    ...
    Real compute(Real drho_dz, Real rho) const {
        return -g_ * drho_dz / rho;
    }

    /// @brief Compute N^2 profile from T, S profiles
    void compute_profile(const VecX& T, const VecX& S, const VecX& z,
                          VecX& N2) const {
        int n = static_cast<int>(T.size());
        N2.resize(n);
