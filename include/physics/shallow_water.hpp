...
public:
    ...
    void source(const VecX& U, const Vec3& position,
                Real bathymetry, const Vec3& grad_bathymetry,
                VecX& S) const;

    // Numerical flux (Riemann solver interface)
    void numerical_flux(const VecX& UL, const VecX& UR,
                        const Vec3& normal, VecX& F_star) const;
...
namespace riemann {
    ...
    void lax_friedrichs(const VecX& UL, const VecX& UR,
                        const Vec3& normal, Real g,
                        VecX& F_star);

    // Roe solver with entropy fix
    void roe(const VecX& UL, const VecX& UR,
             const Vec3& normal, Real g,
             VecX& F_star);

    // HLLC solver
    void hllc(const VecX& UL, const VecX& UR,
              const Vec3& normal, Real g,
              VecX& F_star);
...
private:
    enum class Mode { FPlane, BetaPlane, Spherical };
...
public:
    enum class Model { None, Linear, Quadratic, Manning };
